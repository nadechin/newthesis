...
\begin{align}
p(w_{O,1}，w_{O,2}，w_{O,3}， \ldots ，w_{O,C}|w_{I}) = \prod_{c=1}^{C} \frac{exp({{v^{\prime}}^{\mathrm{T}}_{W_{V}}} \cdot v_{w_{I}})}{\sum_{W_{v} \in{V}}exp({{v^{\prime}}^{\mathrm{T}}_{W_{V}}} \cdot v_{w_{I}})} \label{eq:douzikakuritu}
\end{align}\\
...
y_{c,i} &= \frac{exp(u_{c,i})}{\sum_{v=1}^{V}exp(u_{c,v})} \nonumber \\
        ...
        &= p(w_{i}|w_{I}) \nonumber
\end{align}
と表され，式(\ref{eq:douzikakuritu})で表した確率と同じになることが確認できる．
