...
また，以下の図\ref{fig:uc}は，入力ベクトルが出力ベクトルのどこに含まれているのかを表したものである．\\
\begin{figure}[htbp]
　\begin{center}
...
\begin{align}
\frac{\partial E}{\partial v^{\prime}_{ij}} = \sum_{C=1}^{C} \frac{\partial E}{\partial u_{c,i}} \frac{\partial u_{c,i}}{\partial } \label{eq:wprimeh}
\end{align}
とすることができる．
...
E &= - \sum_{C=1}^{C}(u_{C,w_{C}} - \log \sum_{V=1}^{V} exp(u_{C,V})) \nonumber \\
  &= - \sum_{C=1}^{C} u_{C,w_{C}} + \sum_{C=1}^{C} \log \sum_{V=1}^{V} exp(u_{C,V}) \label{eq:daiikkou}
\end{align}
式(~\ref{eq:daiikkou})の内，$ \sum_{C=1}^{C} u_{C,w_{C}} $を$ u_{c,i} $で微分することを考える．
\begin{align}
\sum_{C=1}^{C} u_{C,w_{C}} = u_{1,w_{1}} + u_{2,w_{2}} + \cdot + u_{C,w_{C}}
\end{align}
